...
public class PlayState extends State
{
    ...
    private static final int BLUE_COUNT = 2;
    ...
    public PlayState(GameStateManager gsm)
    {
        ...
        groundchange = new Texture("backgroundchange.png");
        groundchange.setFilter(Texture.TextureFilter.Linear, Texture.TextureFilter.Linear);
        setground = false;
    ...
    protected void handleInput()
    {
            ...
            if (Gdx.input.getX() > Gdx.graphics.getWidth() / 2) {
                if (character.getPosition().x >= LightDefender.WIDTH - character.getcharacter().getWidth()) {
                    ...
                    if (counter < 6)
                        ...
                        character.setPosition(character.getPosition().add(25, 0, 0));
                    else
        ...
        if (counter <= 4)
            character.setPosition(character.getPosition().add(0, 15, 0));
        else if (counter > 4 && counter <= 9)
            character.setPosition(character.getPosition().add(0,20,0));
        else if (counter > 9 && counter <= 19)
            character.setPosition(character.getPosition().add(0,25,0));
        else if (counter > 19 && counter <= 24) {
            character.setPosition(character.getPosition().add(0, 15, 0));

            if (turquoises.size == 3) {
                turquoises.add(new Turquoise(cam.position.y + cam.viewportHeight / 2 + 500));
                turquoises.add(new Turquoise(cam.position.y + cam.viewportHeight / 2 + 500));
                turquoises.add(new Turquoise(cam.position.y + cam.viewportHeight / 2 + 500));
            }
            ...
            character.setPosition(character.getPosition().add(0, 20, 0));
        else if (counter > 29 && counter <= 69)
            character.setPosition(character.getPosition().add(0,25,0));
        else
            character.setPosition(character.getPosition().add(0,40,0)); //Good luck
    ...
    public void update(float dt)
    {
        ...
        if (counter / 10 == 1 || counter / 10 == 3 || counter / 10 == 5 || counter / 10 == 7 || counter / 10 == 9)
            setground = true;
        else
            setground = false;
        ...
        for (BlueBlock i : blues)
        {
            ...
            if (i.collides(character.getBounds()))
                gsm.set(new GameOverState(gsm, counter, true));
    ...
    public void render(SpriteBatch sb)
    {
        ...
        if (setground == false) {
            ...
            sb.draw(groundchange, groundPos1.x, groundPos1.y);
            sb.draw(groundchange, groundPos2.x, groundPos2.y);
        }

        if (counter > 66 && counter < 70)
        font.draw(sb, "Good Luck!", 310, (cam.position.y));
